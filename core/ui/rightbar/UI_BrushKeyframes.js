global.UI_BrushKeyframes = class extends ve.Class {
	...
	draw () {
		...
		//Return statement
		return veInterface({
			info: veHTML(`<b>Note.</b> Date ranges are currently only applied to Node Brushes to prevent accidental data erasures. Regular brushes can be used on temporary Polygons and transferred over using the <kbd>Node Transfer</kbd> brush.`, {
				style: { padding: 0 }
			}),
			
			apply_date_range: veToggle(this.enabled, {
				name: "Apply Date Range",
				onuserchange: (v) => this.enabled = v,
				tooltip: "Whether to apply brush edits to the given date range."
			}),
			start_date: veDate(start_date, {
				name: "Start Date",
				onuserchange: (v) => this.start_date = v
			}),
			end_date: veDate(end_date, {
				name: "End Date",
